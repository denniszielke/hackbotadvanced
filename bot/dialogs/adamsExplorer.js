...
const adamsHelper = require('../adamsHelper.js');
const messageHelper = require('../messageHelper.js');

// update this to be the name of your facet
const facetName = 'Adams';

module.exports = {
    id: 'adamsExplorer',
    title: 'Explore the ultimate question',
    dialog: [
        (session) => {
            adamsHelper.facetQuery(facetName, (err, result) => {
                if (err) {
                    ...
                    result.forEach(function (facet, i) {
                        ...
                        buttons.push(
                            builder.CardAction.imBack(session, facet.value, `${facet.value}`)
                        );
                    });
        ...
        (session, results) => {
            ...
            adamsHelper.searchQuery(facetValue, (err, result) => {
                if (result) {
                    const message = messageHelper.getCarousel(session, result);
